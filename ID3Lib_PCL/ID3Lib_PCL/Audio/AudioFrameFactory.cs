    ...
    internal static class AudioFrameFactory
    {
        ...
        static public AudioFrame CreateFrame( Stream stream, uint remainingBytes )
        {
            ...
            else
            {
                ...
                Debug.Assert(frameLengthInBytes != null);
        ...
        static private uint GetNextFrameOffset( Stream stream, uint remainingBytes )
        {
            ...
            Debug.WriteLine(String.Format("next frame is {0} bytes further", stream.Position - prevHeaderEnd));
            Debug.WriteLine(header.DebugString);
        ...
        static AudioFrameHeader CreateHeader( Stream stream, uint remainingBytes )
        {
            ...
            do
            {
                ...
                if( parsedHeader.Valid )
                {
                    ...
                    {
                        if( framesSkipped > 0 )
                            Debug.WriteLine(string.Format("total {0} bytes and {1} invalid frame headers skipped to get to the start of a valid frame at stream offset {2}",
                                                            frameStartPos - streamStartpos, framesSkipped, frameStartPos));
                        else
                            Debug.WriteLine(string.Format("total {0} bytes skipped to get to the start of a valid frame at stream offset {1}",
