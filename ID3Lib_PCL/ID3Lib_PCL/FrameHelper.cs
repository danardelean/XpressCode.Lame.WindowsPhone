    ...
    internal class FrameHelper
    {
        ...
        public FrameBase Build(string frameId, ushort flags, byte[] buffer)
        {
            ...
            if (frame.Compression == true)
            {
                ...
                index = 0;
                //DAN
                stream = new Ionic.Zlib.DeflateStream(stream, Ionic.Zlib.CompressionMode.Compress); //new InflaterInputStream(stream);
            }
        ...
        public byte[] Make(FrameBase frame, out ushort flags)
        {
            ...
            if (frame.Compression == true)
            {
                ...
                using (var deflater = new Ionic.Zlib.DeflateStream(temp, Ionic.Zlib.CompressionMode.Decompress))
                {
                    deflater.CopyTo(memoryStream);
                }
