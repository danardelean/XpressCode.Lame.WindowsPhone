...
namespace Mp3Lib
{
    ...
    internal class Mp3FileData
    {
        ...
        public UInt32 AudioStart
        {
            get { return _audioStart; }
            set { _audioStart = value; }
        }
        ...
        public Mp3FileData(Stream stream)
        {
            ...
            if (_sourceStream.Length > UInt32.MaxValue)
            ...
            audioNumBytes = (UInt32)_sourceStream.Length;
            ...
            try
            {
                ...
                id3v1.Deserialize(_sourceStream);
            ...
            {
                _sourceStream.Seek(0, SeekOrigin.Begin);
                tagModel = TagManager.Deserialize(_sourceStream);

                // audio starts after the tag
                _audioStart = (uint)_sourceStream.Position;
            ...
            _audio = new AudioFile(stream, _audioStart, audioNumBytes, _tagHandler.Length);
        ...
        public CacheDataState Update()
        {
            ...
            else
            {
                ...
                else
                {
                    ...
                    //string bakName = Path.ChangeExtension(_sourceStream.FullName, "bak");
        ...
        private void UpdateInSitu(uint tagSizeComplete)
        {
            ...
            //using (FileStream writeStream = _sourceStream.Open(FileMode.Open, FileAccess.ReadWrite, FileShare.Read))
            //{
        ...
        /*private void RewriteFile(FileInfo bakFileInfo)
        {
            ...
            string tempName = Path.ChangeExtension(_sourceStream.FullName, "$$$");
            ...
            try
            {
                FileMover.FileMove(tempFileInfo, _sourceStream, bakFileInfo);
            }
            catch
            {
                tempFileInfo.Delete();
        ...
        /// Always need to re-initialise the mp3 file wrapper if you use it after this runs
        /// </remarks>
        /// <param name="destination">location of backup file - must be on same drive</param>
        private void RewriteFileNoV2tag(Stream writeStream)
        {
            // generate a temp filename in the target's directory
            CopyAudioStream(writeStream);
        ...
        public void CopyAudioStream(Stream writeStream)
        {
